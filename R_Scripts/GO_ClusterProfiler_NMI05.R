...
df <- read.delim("GeneListOnly_CpGOfNMI0.05.txt", header = F) #Gene List in text file
...
myEnrich <- enrichDGN(GeneID$ENTREZID, pvalueCutoff = 0.2) #Disease Pathway
...
myEnrichKegg <- enrichKEGG(GeneID$ENTREZID, organism = "hsa",qvalueCutoff = 0.5, keyType = "ncbi-geneid",pvalueCutoff = 0.7) #KEGGPathway
