...
Feat_Data = vroom("SymbDigiCateg_Methylome_Male.csv")
head(Feat_Data)[1:8,1:8]
FeatData <- Feat_Data %>% select(-c(1,2,3,5,4,7))
colnames(FeatData)[1]
...
write.csv(test_result, "NMI_APPRDX_SymbDigi_Male.csv")
...
colnames(test_result) <- ("MutInf_UPSIT")
test_result$Normalized <- (test_result$MutInf_UPSIT)/(0.6745647505)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_CategWise_SymbDigi_Male.csv")
...
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_HYS_SymbDigi_Male.csv")
...
colnames(test_result) <- ("MutInf_UPSIT")
test_result$Normalized <- (test_result$MutInf_UPSIT)/(0.6733923276)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_WithinPatient_CategWise_SymbDigi_Male.csv")
...
colnames(test_result) <- ("MutInf_UPSIT")
test_result$Normalized <- (test_result$MutInf_UPSIT)/(0.6785902143)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_WithinHealthy_CategWise_SymbDigi_Male.csv")
