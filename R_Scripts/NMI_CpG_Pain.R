...
Feat_Data = vroom("PainCateg_Methylome_APPRDXFilter.csv")
...
colnames(test_result) <- ("MutInf_Pain")
test_result$Normalized <- (test_result$MutInf_Pain)/(0.6030699704)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_APPRDX_Pain.csv")
...
colnames(test_result) <- ("MutInf_Pain")
test_result$Normalized <- (test_result$MutInf_Pain)/(0.847714422)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_CategWise_Pain.csv")
...
###################### 3. Gender [Male, Female] Wise ###################### 
  ...
  colnames(test_result) <- ("MutInf_Pain")
  test_result$Normalized <- (test_result$MutInf_Pain)/(0.6329163173)
  test_result <- arrange(test_result, desc(Normalized))
  write.csv(test_result, "NMI_Gender_Pain.csv")
...
colnames(test_result) <- ("MutInf_Pain")
test_result$Normalized <- (test_result$MutInf_Pain)/(1.159183000)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_HYS_Pain.csv")
...
colnames(test_result) <- ("MutInf_Pain")
test_result$Normalized <- (test_result$MutInf_Pain)/(0.87026048)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_WithinPatient_CategWise_Pain.csv")
...
colnames(test_result) <- ("MutInf_Pain")
test_result$Normalized <- (test_result$MutInf_Pain)/(0.78522456)
test_result <- arrange(test_result, desc(Normalized))
write.csv(test_result, "NMI_WithinHealthy_CategWise_Pain.csv")
