...
df <- read.delim("GeneList_CommonCpG_ChrNMI_9Feats.txt", header = F) #Gene List in text file
...
myEnrich <- enrichDGN(GeneID$ENTREZID, pvalueCutoff = 0.9) #Disease Pathway
...
myEnrichKegg <- enrichKEGG(GeneID$ENTREZID, organism = "hsa", keyType = "ncbi-geneid",pvalueCutoff = 0.2) #KEGGPathway
