...
library(STRINGdb) # For protein-protein interaction data
 ...
 ## similarly as MALE, here also we got only 3 genes, and upon analysis we got 0 hits, so it has no results ##

setwd('S:\\PPMI_RNA\\Processed')
df <- read.csv('DEGs_Result_UpDown.csv')
...
write.csv(results, 'Nodes_Edges_Combined.csv')
...
write.csv(results_20, 'Nodes_Edges_Combined_Top20.csv', row.names = F)

# Plot degree centrality
ggplot(results_20, aes(x = name, y = degree)) +
  geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1))

results_20 = read.csv('Nodes_Edges_Combined_Top20.csv')
# as increasing order, colorful
results_20$name <- factor(results_20$name, levels = results_20$name[order(results_20$degree)])
ggplot(results_20, aes(x = name, y = degree, fill = name)) + geom_bar(stat = 'identity') + theme_bw() +
  theme(legend.position = 'none') + theme(axis.text.x = element_text(angle = 90, hjust = 1, color = 'black')) +
  theme(axis.title = element_text(face = "bold")) + theme(axis.text = element_text(face = "bold", color = 'black')) +
  theme(text = element_text(size = 14)) + theme(plot.title = element_text(face = "bold", size = 18)) +
  ggtitle ("Degree of centrality of Top 20 genes: Combined")
