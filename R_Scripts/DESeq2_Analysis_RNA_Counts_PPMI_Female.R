...
countData <- read.csv('All_Counts_PatAndHealthy_Female.csv', header = TRUE, sep = ",")
head(countData[1:6, 1:8])

metaData <- read.csv('metaDataIR3_OnlyPat_Healthy_PATNO_Filter_Female.csv', header = TRUE, sep = ",")
head(metaData)
...
write.csv(res_list, 'DEGs_Result_Female.csv', row.names = F)
...
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="DEGs of PPMI Data: Patient Vs Healthy Control: Female", 
               xlim=c(-1.5,1.5), ylim=c(0,20)))
# Add colored points: blue if padj<0.01, red if log2FC>1 and padj<0.05)
with(subset(res, padj<.01), points(log2FoldChange, -log10(pvalue), pch=20, col="#F2CC8F"))
with(subset(res, padj<.01 & (log2FoldChange)>0.25), points(log2FoldChange, -log10(pvalue), pch=20, col="#36C3D1")) #Upregulate
