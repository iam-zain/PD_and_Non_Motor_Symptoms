...
countData <- read.csv('All_Counts_PatAndHealthy.csv', header = TRUE, sep = ",")
head(countData[1:6, 1:8])

metaData <- read.csv('metaDataIR3_OnlyPat_Healthy.csv', header = TRUE, sep = ",")
head(metaData)
...
write.csv(res_list, 'DEGs_Result.csv', row.names = F)
...
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="DEGs of PPMI Data: Patient Vs Healthy Control", xlim=c(-1.5,1.5)))
